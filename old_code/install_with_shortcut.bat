@echo off
echo ========================================
echo Install Command Console with Shortcut
echo ========================================
echo.
...
echo Installing Command Console with keyboard shortcut...
echo.

REM Define user CEP extensions directory
set "USER_CEP_DIR=%APPDATA%\Adobe\CEP\extensions"

echo Creating manifest with keyboard shortcut...
echo.

REM Create manifest with keyboard shortcut
...
echo           ^<MainPath^>./command_console_with_shortcut.html^</MainPath^> >> "%SCRIPT_DIR%CSXS\manifest.xml"
echo           ^<ScriptPath^>./jsx/command_processor_with_shortcut.jsx^</ScriptPath^> >> "%SCRIPT_DIR%CSXS\manifest.xml"
echo         ^</Resources^> >> "%SCRIPT_DIR%CSXS\manifest.xml"
...
if %errorLevel% == 0 (
    echo.
    echo ========================================
    echo Command Console with Shortcut Installed!
    echo ========================================
    echo.
    echo The Command Console has been installed with keyboard shortcut support.
    echo.
    echo Next steps:
    echo 1. Restart After Effects completely
    echo 2. Go to Edit ^> Keyboard Shortcuts...
    echo 3. Search for "Command Console"
    echo 4. Assign a shortcut (e.g., Ctrl+Shift+C)
    echo 5. Use the shortcut to open the console and start typing immediately
    echo.
    ...
    echo.
    echo The console will automatically focus the input field when opened.
    echo.
) else (
    echo.
    echo ========================================
