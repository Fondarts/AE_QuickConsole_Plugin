function processCommand(command) {
    try {
        ...
        if (cmd === "delete") {
            ...
            app.endUndoGroup();
            return "Layers deleted";
        }
        
        if (cmd === "hide") {
            app.beginUndoGroup("Hide");
            for (var i = 0; i < sel.length; i++) {
                sel[i].enabled = false;
            }
            app.endUndoGroup();
        ...
        // Try to apply effect by name
        try {
            app.beginUndoGroup("Apply Effect");
            var effectAdded = false;
            for (var i = 0; i < sel.length; i++) {
                try {
                    sel[i].property("ADBE Effect Parade").addProperty(command);
                    effectAdded = true;
                } catch (e) {
                    // Effect not found, continue
                }
            }
            app.endUndoGroup();
            if (effectAdded) {
                return "Effect applied: " + command;
            }
        } catch (e) {
