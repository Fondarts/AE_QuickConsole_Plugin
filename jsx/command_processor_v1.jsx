// Command Processor v1.0 - Auto-scan version
// This version automatically scans for effects on startup
...
function initializeEffects() {
    ...
    try {
        ...
        try {
            var debugFileName = "AE_Plugin_Scan_Debug_v1_" + new Date().getFullYear() + "_" + (new Date().getMonth() + 1) + "_" + new Date().getDate() + ".txt";
            var desktopPath = Folder.desktop.fsName;
            var debugFilePath = desktopPath + "/" + debugFileName;
            var debugFile = new File(debugFilePath);
            
            if (debugFile.open("w")) {
                debugFile.write("After Effects Plugin Scan Debug v1.0\n");
                debugFile.write("====================================\n");
...
// Test function
function testJSX() {
    return "JSX v1.0 working! Found " + (allEffects ? allEffects.length : 0) + " effects";
}
...
function processCommand(command) {
    try {
        ...
        if (cmd === "test") {
            return "Plugin v1.0 is working! Available commands: list effects, [effect name]";
        }
        ...
        if (cmd === "list effects") {
            try {
                ...
                // Save to file
                var fileName = "AE_Effects_List_v1_" + new Date().getFullYear() + "_" + (new Date().getMonth() + 1) + "_" + new Date().getDate() + ".txt";
                var desktopPath = Folder.desktop.fsName;
                var filePath = desktopPath + "/" + fileName;
                var file = new File(filePath);
                
                if (file.open("w")) {
                    file.write("After Effects Effects List v1.0\n");
                    file.write("===============================\n");
